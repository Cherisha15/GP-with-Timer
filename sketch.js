...
function draw() {
  ...
  if(gameState === "start0"){
    ...
    strokeWeight(3);
    textSize(28);
    text("Ivy was born with the magical power of transformation. ", 0, 30);
    text("She eventually gets curious about her true identity as it is not .", 0 ,55);
    text("normal. So she sets on an adventure to find her true identity." , 0, 80);

    fill("red");
    text("Press Arrow keys to move the girl", 0, 100);
...
bird.visible= false;
  ...
  fill("red");
  text("Ivy transforms each time your score is in the multiple of 100." , 0, 50);
  ...
  if(keyDown("s")){
    ...
    if(score>=300){
      image(winImg, 0, 0, 900, 600);
       ...
       strokeWeight(3);
       textSize(25);
       text("Ivy is actually an angel sent to earth by god to ensure peace and happiness", 20, 550);
       text(" on earth 👼", 20, 570);
